...
if ($organisation === false) {
    print_error('invalidorg', 'local_tdmcobrand', new moodle_url('managerules.php'));
}
...
try {
    ...
    $rule = new local_tdmcobrand_rule();
}

$mform = new local_tdmcobrand_rule_form(null, array(
    'fileopts'     => $fileopts,
    'framework'    => $framework,
...
if ($data = $mform->get_data()) {
    ...
    file_save_draft_area_files($data->applylogo, $context->id, 'local_tdmcobrand', 'applylogo', $rule->get_id());
    ...
    if (isset($record->id)) {
        ...
        file_prepare_draft_area($draftareaid, $context->id, 'local_tdmcobrand', 'applylogo', $record->id, $fileopts);
    ...
    echo $OUTPUT->header(),
         $OUTPUT->heading(get_string('editingorgx', 'local_tdmcobrand', (object) array(
